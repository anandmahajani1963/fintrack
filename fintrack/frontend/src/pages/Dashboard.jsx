...
// Changes: Fixed password passing, added large expenses panel
// ============================================================
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label={last?.month || 'Last month'} value={fmt(last?.total || 0)}
                  sub={<span className={`flex items-center gap-1 ${momColor}`}>
                    <MomIcon size={12} />
                    {momDelta >= 0 ? '+' : ''}{fmt(momDelta)} vs prior
                  </span>}
      ...
      <Card>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={months} margin={{ top: 5, right: 10, bottom: 5, left: 10 }}>
            ...
            <Tooltip formatter={v => [fmtDec(v), 'Spend']}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div key={`${cat.category}-${cat.subcategory}`}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-gray-900 dark:text-white font-semibold">
                    {fmt(cat.total)}<span className="text-gray-400 font-normal ml-1">({cat.pct}%)</span>
                  </span>
                </div>
        ...
        <Card>
          <SectionTitle>Large Expenses (&gt;$500)</SectionTitle>
          {!large || large.count === 0
            ? <p className="text-sm text-gray-400">No large expenses this year</p>
            : <div className="space-y-2">
                {large.items.slice(0, 6).map(t => (
