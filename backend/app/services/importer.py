...
CARD_CONFIGS = {
    ...
    # We flip sign so expenses become positive, then skip credits (now negative)
    "chase_checking": {
        "date_cols":   ["Posting Date", "Date"],
        ...
        "flip_sign":   False,
        "type_col":    "Type",
        "debit_types": ["DEBIT_CARD", "ACH_DEBIT", "CHASE_TO_PARTNERFI",
                        "QUICKPAY_DEBIT", "ATM", "CHECK_PAID", "WIRE_OUT",
                        "DEBIT"],
    },
}
...
def parse_csv(
    ...
    for idx in range(len(raw_df)):
        ...
        # For chase_checking: filter by transaction type and make positive
        if cfg.get("type_col") and cfg.get("debit_types"):
            type_col = cfg["type_col"]
            if type_col in raw_df.columns:
                row_type = str(raw_df[type_col].iloc[idx]).strip()
                if row_type not in cfg["debit_types"]:
                    result.skipped_count += 1
                    continue
            amount = abs(amount)
