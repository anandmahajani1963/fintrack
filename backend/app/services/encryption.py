...
def verify_key_check(key: bytes, key_check: bytes) -> bool:
    """
    Verify that a derived key matches the stored key_check.
    Returns True if the key is correct, False otherwise.
    """
    from nacl.exceptions import CryptoError
    box = SecretBox(key)
    try:
        decrypted = box.decrypt(key_check)
        return decrypted == _KEY_CHECK_PLAINTEXT
    except CryptoError:
        return False
