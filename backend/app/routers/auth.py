...
def reset_password(
    ...
    # Delete all user data (zero-knowledge: cannot re-encrypt without old password)
    uid = str(row.user_id)
    ...
    db.execute(text("DELETE FROM user_keys WHERE user_id = :uid"), {"uid": uid})
    db.execute(text("DELETE FROM expense_thresholds WHERE user_id = :uid"), {"uid": uid})
    db.execute(text("DELETE FROM categories WHERE user_id = :uid"), {"uid": uid})
    # Update password hash
    db.execute(text("""
        UPDATE users SET password_hash = :hash
        WHERE id = :uid
    """), {"hash": hash_password(body.password), "uid": uid})
    # Generate new user_keys
    new_salt = generate_kdf_salt()
    new_key = derive_key(body.password, new_salt)
    new_key_check = make_key_check(new_key)
    db.execute(text("""
        INSERT INTO user_keys (user_id, kdf_salt, key_check)
        VALUES (:uid, :salt, :check)
    """), {"uid": uid, "salt": new_salt, "check": new_key_check})
    import uuid
    _seed_categories_for_user(uuid.UUID(uid), db)
    # Mark token as used
    db.execute(text("""
    ...
    """), {"token": body.token})
    db.commit()
    logger.info(f"Password reset (data cleared) for user {uid}")
    return {"message": "Password reset successfully. Your previous financial data has been cleared. You can now log in with your new password."}


# ============================================================
# Change Password (re-encrypts user_keys with new key)
# ============================================================
...
def change_password(
    ...
    """Change password with full re-encryption of user_keys."""
    old_password = req.headers.get("X-Fintrack-Password")
    if not old_password:
        raise HTTPException(status_code=400, detail="Current password header required.")
    ...
    # Get current user_keys
    user_key = db.query(UserKey).filter(UserKey.user_id == current_user.id).first()
    if not user_key:
        raise HTTPException(status_code=400, detail="User key not found.")

    # Derive old key and verify
    old_key = derive_key(body.old_password, bytes(user_key.kdf_salt))
    from app.services.encryption import verify_key_check
    if not verify_key_check(old_key, bytes(user_key.key_check)):
        raise HTTPException(status_code=400, detail="Current password is incorrect.")
    ...
    # Update user_keys with new salt and key_check
    db.execute(text("""
        UPDATE user_keys
        SET kdf_salt = :salt, key_check = :check, updated_at = NOW()
        WHERE user_id = :uid
    """), {
        "salt": new_salt,
        "check": new_key_check,
    ...
    """), {"hash": hash_password(body.new_password), "uid": str(current_user.id)})
